...
pub fn main() !void {
    ...
    var user_text: [100]u8 = undefined;

    // Get input from user
    try stdout.print("Enter some text (max 99 chars): ", .{});
    if (try stdin.readUntilDelimiterOrEof(&buffer, '\n')) |user_input| {
        // Create another stack variable with user's text
        @memcpy(user_text[0..user_input.len], user_input);
        user_text[user_input.len] = 0; // null terminate
    }
    ...
    try stdout.print("buffer address: {*}\n", .{&buffer});
    try stdout.print("user_text address: {*}\n", .{&user_text});
